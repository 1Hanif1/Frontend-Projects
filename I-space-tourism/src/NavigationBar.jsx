    ...
    <div className="font-barlow text-white absolute backdrop-blur-2xl bg-white/5 right-0 top-0 h-screen md:h-max w-7/12 md:w-8/12 lg:w-max flex-col md:px-10 lg:static lg:px-0 hidden md:flex">
      ...
      <div className=" mt-28 md:mt-0 uppercase tracking-[.27em] text-base md:flex justify-between items-center lg:w-max">
        <div className="mb-10 flex md:relative md:my-10 lg:ml-28 lg:mr-20 cursor-pointer">
          ...
          <span className={`ml-auto w-1 md:absolute md:w-full md:h-1 -bottom-10 bg-white block opacity-0 ${page === 'home' && 'opacity-100'}`}></span>
        </div>
        <div className="mb-10 flex md:relative md:my-10 lg:mr-20 cursor-pointer">
          <span className="font-bold ml-7 mr-5 tracking-[.27em] md:hidden lg:ml-0 lg:mr-2 lg:block">01</span>Destination<span className={`ml-auto w-1 md:absolute md:w-full md:h-1 -bottom-10 bg-white block opacity-0 ${page === 'destination' && 'opacity-100'}`}></span>
        </div>
        <div className="mb-10 flex md:relative md:my-10 lg:mr-20 cursor-pointer">
          <span className="font-bold ml-7 mr-5 tracking-[.27em] md:hidden lg:ml-0 lg:mr-2 lg:block">02</span>Crew<span className={`ml-auto w-1 md:absolute md:w-full md:h-1 -bottom-10 bg-white block opacity-0 ${page === 'crew' && 'opacity-100'}`}></span>
        </div>
        <div className="mb-10 flex md:relative md:my-10 lg:mr-40 cursor-pointer">
          <span className="font-bold ml-7 mr-5 tracking-[.27em] md:hidden lg:ml-0 lg:mr-2 lg:block">03</span>Technology<span className={`ml-auto w-1 md:absolute md:w-full md:h-1 -bottom-10 bg-white block opacity-0 ${page === 'technology' && 'opacity-100'}`}></span>
        </div>
      </div>
    </div>
  </nav>;
